    ...
    <div className="tn-shell" data-testid="app-shell" dir={dir}>
      <aside className="tn-shell__rail">
        <div className="tn-brand">
          <span className="tn-brand__mark">
            <RadioTower size={22} />
          </span>
          <span className="tn-brand__text">
            <strong>{brand}</strong>
            <small>Edge Platform</small>
          </span>
        </div>
        <nav className="tn-nav" aria-label="Primary navigation">
          {navigation.map((item) => (
            <button
              className={item.active ? 'tn-nav__item is-active' : 'tn-nav__item'}
              key={item.id}
              type="button"
            >
        ...
        <div className="tn-shell__locale">
          <Languages size={17} />
