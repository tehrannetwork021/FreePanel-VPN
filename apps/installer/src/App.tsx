      ...
      <section className="installer__hero">
        <div className="installer__copy">
          ...
          <div className="trust-row">
            <span>
              <ShieldCheck size={16} /> {t('installer.token.neverStored')}
            </span>
            <span>
        ...
        <div className="steps" aria-label="Installation steps">
          <div className="step-card is-current">
            <b>01</b>
            <span>{t('installer.token.step1')}</span>
          </div>
          <div className="step-card">
            <b>02</b>
            <span>{t('installer.token.step2')}</span>
          </div>
          <div className="step-card">
            <b>03</b>
            <span>{t('installer.token.step3')}</span>
          </div>
        </div>
      </section>
      ...
      <section className="installer-card">
        ...
        <form onSubmit={handleInstall} className="token-form">
          <label htmlFor="cloudflare-token">
            <KeyRound size={17} /> Cloudflare API Token
          </label>
          ...
          {state === 'success' ? (
            <p className="installer-message is-success">
              <Check size={16} /> {t('installer.progress.done')}
            </p>
          ) : null}
        </form>
        ...
        <div className="progress-strip" aria-label="Automatic installation pipeline">
          {[
            t('installer.progress.verify'),
            t('installer.progress.account'),
            t('installer.progress.kv'),
            t('installer.progress.worker'),
          ].map((label, index) => (
            <div
              className={
                state === 'success'
                  ? 'progress-item is-done'
                  : state === 'installing' && index === 0
                    ? 'progress-item is-active'
                    : 'progress-item'
              }
              key={label}
            >
              <span>{state === 'success' ? <Check size={13} /> : index + 1}</span>
              <small>{label}</small>
            </div>
          ))}
        </div>
