...
### Stage 1 — Deploy the installer (once)
...
2. Sign in to your free Cloudflare account and approve the deployment.
3. Cloudflare provisions the installer Worker and gives you a URL like `https://tehran-network-installer.<account-subdomain>.workers.dev`. Open it and keep it.

> The installer is a local page inside your own account; your token is never sent to or stored by any Tehran Network server.

### Stage 2 — Generate the key and install the panel
...
3. Paste the token into the installer and verify; your accounts are discovered.
4. Choose the account, set a Worker name (default `tehran-network-edge`) and an owner admin password for the panel.
...
6. You receive `https://<worker-name>.<account-subdomain>.workers.dev`. Open it, enter the same admin password and collect your configs.
